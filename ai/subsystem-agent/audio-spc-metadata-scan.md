...
- Deep Scan reads valid SPC ID666 and xID6 metadata directly through `SPCMetadataReader`; it does not create a libgme inspector for ordinary tagged SPC files.
...
- Deep Scan materializes an archive set once, then inspects its members with bounded workers. Results are restored to archive-entry order before persistence.
