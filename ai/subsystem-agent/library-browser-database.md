...
- `LibraryConsoleResolver` owns persisted sidebar-console selection. Collection folders are authoritative by default and canonical aliases such as `Playstation` become `Sony PlayStation`; `Prefer Embedded Console Tags` reverses that priority. Decoder routing remains independent, and the default keeps vgmstream container labels from overriding recognized collection folders.
- Changing the console-tag source rewrites game/system identity in bounded 5,000-row batches inside one savepoint, dirties only the Games projection, and reloads the sidebar without rescanning or decoding audio. Schema 21 performs the same identity repair once for schema-20 databases.
...
- The Games browser has a separate covering `tracks_game_sidebar_index` for grouped startup reads and unlinked-source filtering. Schemas 16 through 20 upgrade in place to schema 21 by applying only their missing index, data-repair, scan-staging, and sidebar-identity steps; other unsupported schemas are reset.
