...
- Changing the console-tag source rewrites game/system identity in bounded 5,000-row batches inside one savepoint, dirties only the Games projection, and reloads the sidebar without rescanning or decoding audio. Schema 22 performs the same identity repair once for schema-21 databases.
...
- The Games browser has a separate covering `tracks_game_sidebar_index` for grouped startup reads and unlinked-source filtering. Schemas 16 through 21 upgrade in place to schema 22 by applying only their missing index, data-repair, scan-staging, and sidebar-identity steps; schema 22 rewrites persisted identity with the shared v1 rules. Other unsupported schemas are reset.
