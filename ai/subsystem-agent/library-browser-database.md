...
- Changing the console-tag source rewrites game/system identity in bounded 5,000-row batches inside one savepoint, dirties only the Games projection, and reloads the sidebar without rescanning or decoding audio. The schema 23 upgrade performs the same identity repair for schema-21 databases; schema-22 databases already contain it.
