...
- Games rows retain `root_id`. Folder-first activation binds `root_id + browser_game + browser_system` to preserve the scanner index; metadata-first activation uses its matching read-only console expression. Never merge roots during either projection or activation.
...
- [CatalogBrowser.swift](/Users/john/Downloads/Code/CocoaSpice/Sources/CocoaSpice/App/CatalogBrowser.swift)
