...
- A sidebar playlist publishes its SQLite projection immediately: row identity, cached metadata, and column hints never wait on a decoder or archive extraction. Missing optional metadata may then hydrate in memory through bounded workers. CocoaSpice never writes hydration results to the MediaScanner catalog; inspection must not structurally reload the queue or delay playback.
