...
- A sidebar playlist is a direct SQLite projection: its rows, cached metadata, and column hints come from the scan database. Building or revisiting that playlist must not inspect a decoder, materialize an archive member, or touch source files. A blank stored duration remains blank until a scan/deep scan refreshes it.
