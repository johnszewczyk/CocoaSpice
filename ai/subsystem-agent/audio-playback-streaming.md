...
- Native transport operations use one final-mixer transition policy: a 24 ms half-cosine duck to silence, graph/decoder replacement while muted, then a 24 ms restore to the user's app-volume setting. It covers pause/resume, seek, stop, track changes, and Long Play decoder reconfiguration without changing PCM or system volume.
...
- The optional toolbar spectrum uses a 4,410-frame mixer tap (10 Hz at 44.1 kHz) and a 4,096-point real FFT. 10/20/40 bars mean 1/2/4 bands per octave from 20 Hz through 20.48 kHz. A fixed AppKit view draws all bars in one pass at 45 FPS; do not restore a SwiftUI `ForEach` titlebar graph, because its per-bar layout dominates CPU. Disabled/stopped spectrum removes the tap and timer.
