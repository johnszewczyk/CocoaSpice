...
- Native transport operations use one render-boundary transition policy: an atomic 10 ms linear de-click envelope applies sample-by-sample after ring-buffer read, graph/decoder replacement occurs only after it reaches silence, then a fresh primed source restores over the same envelope. Every fresh source, including the first start after launch/Stop and output-route recovery, begins muted. The persisted App Volume remains at the final mixer and never changes during transitions. It covers pause/resume, seek, stop, track changes, and Long Play decoder reconfiguration without rewriting track PCM or changing system volume.
- Pause keeps the configured `AVAudioEngine` output route and realtime callback live. After the de-click ramp reaches silence, an atomic render hold makes the callback write zeros without reading the ring buffer; decoder refill is suspended. Resume releases that hold and restores the 10 ms envelope, preserving the exact pending PCM frame without a route stop/start.
...
- Archive playback materialization is policy-owned. Cache-on uses a durable 2 GB/4 GB/8 GB/16 GB LRU store (2 GB default); cache-off uses an isolated disposable root that is discarded when playback stops. Both modes reserve 1 GB free disk space and reject a materialization that cannot fit the active limit. Selected-entry formats, including SPC, cache only the requested TAR+Zstandard member rather than expanding an unrelated complete archive. Scan scratch remains separate and is reclaimed on the next launch if an interrupted prior scan stranded a root.
...
- A sidebar playlist publishes its SQLite projection immediately: row identity, cached metadata, and column hints never wait on a decoder or archive extraction. Missing optional metadata then hydrates in the background through bounded workers. A loose-source or archive-member result writes back only when its path/member/subtrack and source size/modification fingerprint still match the live database row; stale hydration is discarded. This metadata-only update must not structurally reload the queue or delay playback.
...
- Only Stop and output-route recovery stop and reset the graph. Track and seek replacement keep it warm; an in-flight read must never republish pre-clear audio into the next stream generation.
