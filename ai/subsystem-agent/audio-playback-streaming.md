...
- Native transport operations use one final-mixer transition policy: a 24 ms half-cosine duck to silence, graph/decoder replacement while muted, then a 24 ms restore to the user's app-volume setting. Every fresh source, including the first start after launch/Stop and output-route recovery, begins muted and restores only after priming. It covers pause/resume, seek, stop, track changes, and Long Play decoder reconfiguration without changing PCM or system volume.
