...
- Pause keeps the configured `AVAudioEngine` output route and realtime callback live. After the de-click ramp reaches silence, an atomic render hold makes the callback write zeros without reading the ring buffer; decoder refill is suspended. Resume releases that hold and restores the 24 ms envelope, preserving the exact pending PCM frame without a route stop/start.
