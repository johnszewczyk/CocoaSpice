...
- Deep Scan derives archive extraction from the decoder module's materialization policy. Modules with independent members, including SPC, materialize every selected playable member with one extractor invocation per archive rather than launching one process per member. GSF, 2SF, PSF/PSF2, and USF-family members reuse one complete-set extraction per archive so sibling libraries remain available; only LazyUSF additionally prepares extensionless library aliases.
