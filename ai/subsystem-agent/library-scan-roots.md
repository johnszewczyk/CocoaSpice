...
- A new scan replaces live root inventory and track rows, while preserving sources marked unlinked. A rediscovered unlinked source is restored before selection so its existing fingerprint, archive signature, and metadata can be reused; only Purge Database or Clean Unlinked permanently removes those rows.
