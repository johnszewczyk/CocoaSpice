...
| `vgmstream TXTP` | `txtp` | Manifests materialize the complete archive set. Flat JoshW Wwise archives receive cache-only hard-link aliases for uniquely referenced sibling paths before vgmstream opens them; source archives remain unchanged. Resident Evil 3 is the regression fixture. Wwise `BKHD` `.bnk` event banks are indexed as non-playable resources; their TXTP/WEM content is the playable representation. |
