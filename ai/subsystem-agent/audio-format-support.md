...
| `vgmstream TXTP` | `txtp` | Playback and metadata scans materialize the complete archive set. Flat archives receive cache-only hard-link aliases for uniquely referenced sibling paths before vgmstream opens them; Windows `\\` references are normalized to archive-relative `/` paths. Source archives remain unchanged. Resident Evil 3 and Jenga World Tour are regression fixtures. Wwise `BKHD` `.bnk` event banks are indexed as non-playable resources; their TXTP/WEM content is the playable representation. |
