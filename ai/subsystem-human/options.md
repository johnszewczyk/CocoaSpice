...
- Sidebar Options: Group by Console sorts the Database game list into consoles. Recognized collection filename tags, then collection folders, are the default console source; Prefer Embedded Console Tags switches existing and future database rows to normalized decoded console metadata without rescanning. Files Disclosure Gap sets 0–16 pt spacing between folder triangles and names in Files view. Files Child Indent is a numeric 0–32 pt field that offsets every Files-view child level; its default 8 pt is about one character at the default font size. Hide File Extensions changes only Files-view labels, never filenames stored by the database or passed to playback.
