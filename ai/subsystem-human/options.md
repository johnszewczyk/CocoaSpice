...
- Test Links checks only whether each unique indexed source path exists, then marks confirmed-missing sources unlinked and hides them while retaining their tracks, metadata, and scan inventory. It does not open archives or read metadata. Rediscovery restores an unlinked source; only Clean Unlinked or Reset Database permanently discards it.
