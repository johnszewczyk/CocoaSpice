...
- Group by Console: Options can group games under expandable console headings. Collection console folders take priority by default and normalize names such as `Playstation` to `Sony PlayStation`; `Prefer Embedded Console Tags` reverses that priority.
- Game names: use the inspected game tag when present, otherwise the archive filename for archived tracks or immediate parent-folder name for loose tracks.
