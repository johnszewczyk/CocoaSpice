...
- Group by Console: Options can group games under expandable console headings. `Prefer Folders over Metadatas` uses the scanner's recognized parent console folder (including the parent of a `.tar.zst` game archive) before embedded console metadata; disabling it reverses that source priority. Changing it reloads the read-only sidebar without rescanning or rewriting the catalog.
